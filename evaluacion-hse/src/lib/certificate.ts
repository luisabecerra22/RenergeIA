...
function textoMultilineaCentrado(
  ...
  for (const palabra of palabras) {
    ...
    if (font.widthOfTextAtSize(prueba, size) > maxAncho && lineaActual) {
  ...
  for (const linea of lineas) {
    textoCentradoEn(page, linea, centerX, y, font, size, color);
    y -= lineHeight;
  }
  return y;
}
...
export async function generarCertificado(intento: Intento): Promise<Uint8Array> {
  ...
  // Nombre de la capacitación en negrita (multilínea si es largo)
  const temaTexto = `"${intento.tema}"`;
  const maxAnchoTema = 340;
  const yDespuesTema = textoMultilineaCentrado(page, temaTexto, centerX, 228, fontBold, 15, maxAnchoTema, AZUL);

  // Resultado obtenido
  const notaTexto = `Resultado obtenido: ${intento.nota.toFixed(1)} / 5.0`;
  textoCentradoEn(page, notaTexto, centerX, yDespuesTema - 5, fontRegular, 13, GRIS);

  // Fecha de expedición
  const fechaTexto = `Expedido el ${fechaLarga(intento.presentadoEn)}`;
  textoCentradoEn(page, fechaTexto, centerX, yDespuesTema - 30, fontItalic, 13, GRIS);
